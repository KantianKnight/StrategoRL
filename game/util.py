...
def convert(game_line, is_left):
    ...
    # New for Stratego Tiny
    if len(arr) == 0:
        return temp_inner_arr
...
def create_initial_positions_from_human_data(player1_string, player2_string, game_version_config: dict):
    # New function for Tiny Stratego
    if game_version_config['rows'] == 4:
        return create_initial_positions_for_tiny(player1_string, player2_string)
...
# New function for Tiny Stratego
def create_initial_positions_for_tiny(player1_string, player2_string):
    correct_board_shape = (4, 4)
    initial_piece_map1 = np.zeros(shape=correct_board_shape, dtype=INT_DTYPE_NP)
    initial_piece_map2 = np.zeros(shape=correct_board_shape, dtype=INT_DTYPE_NP)
    positions = np.asarray([initial_piece_map1, initial_piece_map2])
    ...
    for player in range(2):
        for i in range(4):
            positions[player][0][i] = firstFours[player][i]
...
def get_random_human_init_fn(game_version, game_version_config, procedural_env=None):
    ...
    ## for tiny
    elif game_version in [GameVersions.TINY.value]:
        from stratego_env.game.inits.tiny_human_inits import TINY_INITS as HUMAN_INITS
    else:
